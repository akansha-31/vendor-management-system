...
class TestPurchaseOrderViewSet(TestCase):
    ...
    def setUp(self) -> None:
        ...
        now = timezone.now()
        self.purchase_order = PurchaseOrder.objects.create(po_number='123', delivery_date=now, items={"1": "one"}, quantity=2, issue_date=now, vendor=self.vendor)
        return super().setUp()
    ...
    def test_create_order(self):
        ...
        response = self.client.post('/api/purchase_orders/', payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_delete_order(self):
        response = self.client.delete(f'/api/purchase_orders/{self.vendor.id}/')
